...
if (!function_exists('trx_addons_theme_panel_load_scripts')) {
	...
	function trx_addons_theme_panel_load_scripts() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='trx_addons_theme_panel') {
            ...
            if( is_rtl() ) {
                wp_enqueue_style( 'trx_addons-msgbox-rtl', trx_addons_get_file_url('js/msgbox/msgbox-rtl.css'), array(), null );
            }
...
if ( !function_exists( 'trx_addons_is_theme_activated' ) ) {
	function trx_addons_is_theme_activated( $suppress_filters = false ) {
		$activated = get_option( sprintf( 'trx_addons_theme_%s_activated', get_option( 'template' ) ) ) == 1
		...
		if (is_admin() && isset($_REQUEST['page']) && $_REQUEST['page']=='trx_addons_theme_panel') {
			...
			if ( !empty( $nonce ) ) {
				...
				// Check code
					...
					$upgrade_url = sprintf(
						trx_addons_get_protocol() . '://upgrade.themerex.net/upgrade.php?key=%1$s&src=%2$s&theme_slug=%3$s&theme_name=%4$s&domain=%5$s&rnd=%6$s&action=check',
						urlencode( $code ),
						urlencode( $theme_info['theme_pro_key'] ),
						urlencode( $theme_info['theme_slug'] ),
						urlencode( $theme_info['theme_name'] ),
						urlencode( trx_addons_remove_protocol( get_home_url(), true ) ),
						mt_rand()
					);
		...
		<div id="trx_addons_theme_panel_section_<?php echo esc_attr($tab_id); ?>" class="trx_addons_tabs_section">
			...
			<div class="trx_addons_theme_panel_theme_<?php echo esc_attr($theme_status); ?>">
				...
				if ( $theme_status == 'active' && trx_addons_is_theme_activated(true) ) {
					...
					<?php do_action('trx_addons_action_theme_panel_deactivation_form'); ?>
					</div><?php
			...
			<div class="trx_addons_theme_panel_section_form_field">
				<label>
					...
					<span class="trx_addons_theme_panel_section_form_field_label"><?php
						echo wp_kses_post(sprintf( __('Your data is stored and processed in accordance with our "%s".', 'trx_addons'),
...
if ( !function_exists( 'trx_addons_remove_theme_activated' ) ) {
	function trx_addons_remove_theme_activated() {
		...
		delete_option( sprintf( 'envato_purchase_code_%s', $template ) );
		delete_option( sprintf( 'envato_access_token_%s', $template ) );
	}
}
	...
	function trx_addons_theme_panel_deactivate_theme() {
		if (is_admin() && isset($_REQUEST['page']) && $_REQUEST['page']=='trx_addons_theme_panel') {
			...
			if ( $action == 'deactivate' && ! empty( $nonce ) ) {
				...
				$code = get_option( sprintf( 'purchase_code_%s', get_option( 'template' ) ) );
				...
				if ( !wp_verify_nonce( $nonce, admin_url() ) ) {
					...
					$theme_info  = trx_addons_get_theme_info();
					$upgrade_url = sprintf(
						trx_addons_get_protocol() . '://upgrade.themerex.net/upgrade.php?key=%1$s&src=%2$s&theme_slug=%3$s&theme_name=%4$s&domain=%5$s&action=deactivate',
						urlencode( $code ),
						urlencode( $theme_info['theme_pro_key'] ),
						urlencode( $theme_info['theme_slug'] ),
						urlencode( $theme_info['theme_name'] ),
					...
					if ( is_serialized( $result ) ) {
						...
						if ( $result['data'] === -1 ) {
							trx_addons_set_admin_message(__('The activation server is not responding! Theme is not deactivated!', 'trx_addons'), 'error');
							...
							trx_addons_set_admin_message(
								! empty( $result['error'] )
									? $result['error']
									: __("Your purchase code is invalid! Theme is not deactivated!", 'trx_addons'),
								'error'
							);
						}
					} else {
						trx_addons_set_admin_message(__('Activation server unexpected response! Theme is not deactivated!', 'trx_addons'), 'error');
					}
				}
			}
		}
...
// Display the theme deactivation form
if ( !function_exists( 'trx_addons_theme_panel_deactivation_form' ) ) {
	add_action('trx_addons_action_theme_panel_deactivation_form', 'trx_addons_theme_panel_deactivation_form');
	function trx_addons_theme_panel_deactivation_form() {
		?>
		<div class="trx_addons_theme_panel_section_license_buttons">
			<form action="<?php echo esc_url(get_admin_url(null, 'admin.php?page=trx_addons_theme_panel')); ?>" name="trx_addons_theme_panel_deactivate_form" method="post">
				...
				<input type="submit" class="trx_addons_button trx_addons_theme_panel_license_disconnect"
					   value="<?php esc_attr_e('Disconnect domain', 'trx_addons'); ?>"
					   onclick="<?php
					   echo esc_js( 'return confirm("' . esc_attr__("After the domain disconnection, you won't be able to install demo data and bundled plugins, as well as receive support for the theme installed on this domain.", "trx_addons") . '") || false' );
					   ?>"
				>
			</form>
		</div>
